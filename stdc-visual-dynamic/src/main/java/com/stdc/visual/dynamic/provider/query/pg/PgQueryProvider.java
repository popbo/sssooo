...
//                        //默认为常规，无小数位
//                        String format = PgConstants.DEFAULT_INT_FORMAT;
//                        if (ObjectUtils.isNotEmpty(measureFormatCondition)){
//                            switch (measureFormatCondition.getFormat()){
//                                case 0:
//                                    format = PgConstants.DEFAULT_INT_FORMAT;
//                                    break;
//                                case 1:
//                                    format = PgConstants.DEFAULT_ONE_FORMAT;
//                                    break;
//                                case 2:
//                                    format = PgConstants.DEFAULT_FLOAT_FORMAT;
//                                    break;
//                                case 3:
//                                    format = PgConstants.DEFAULT_THREE_FORMAT;
//                                    break;
//                                case 4:
//                                    format = PgConstants.DEFAULT_FOUR_FORMAT;
//                                    break;
//                                default:
//                                    break;
//                            }
//                        }
//                        fieldName = String.format(PgConstants.SUM, String.format(PgConstants.CAST, originField, format));
//                    }
//                }
//            }else {
//                //度量数据格式
...
//    }
        ...
        for (ChartFieldCustomFilterDTO request : requestList) {
            ...
            if (field.getDeType() == DeTypeConstants.DE_TIME) {
                ...
                if (field.getDeExtractType() == DeTypeConstants.DE_INT || field.getDeExtractType() == DeTypeConstants.DE_FLOAT || field.getDeExtractType() == DeTypeConstants.DE_BOOL) {
                    ...
                    String cast = String.format(PgConstants.CAST, originName, PgConstants.DEFAULT_INT_FORMAT) + "/1000";
                    //转换时间戳为具体格式时间 FROM_UNIXTIME(CAST(field AS DECIMAL(20,2))/1000 ,'%Y-%m-%d %H:%i:%S')
                    whereName = String.format(PgConstants.FROM_UNIXTIME, cast, PgConstants.DEFAULT_DATE_FORMAT);
                }
                //判断de抽取类型为 1
            ...
            for (ChartCustomFilterItemDTO filterItemDTO : filter) {
                ...
                if (whereTerm.contains("IN") || whereTerm.contains("in")){
                    ...
                    whereValue = String.format(PgConstants.WHERE_VALUE_BETWEEN,betweenValueS[0],betweenValueS[1]);
                } else {
                ...
                if (whereTerm.contains("IN") || whereTerm.contains("in")){
                    ...
                    if (StringUtils.containsIgnoreCase(filterItemDTO.getTerm(), "in")) {
                        ...
                        whereValue = "'%" + value + "%'";
                            ...
                            || StringUtils.containsIgnoreCase(filterItemDTO.getTerm(),"gt")
                            || StringUtils.containsIgnoreCase(filterItemDTO.getTerm(),"ge")){
                        whereValue = String.format(PgConstants.WHERE_VALUE_BLANK, value);;
                    }else {
                        //转换 `whereValue`
                        whereValue = String.format(PgConstants.WHERE_VALUE_VALUE, value);
                    }
    ...
    public String createQueryTableWithPage(String table, List<DatasetTableField> fields, Integer page, Integer pageSize, Integer realSize, boolean isGroup, Datasource ds, List<ChartFieldCustomFilterDTO> fieldCustomFilter) {
        return createQuerySQL(table, fields, isGroup,null, fieldCustomFilter) + " LIMIT " + realSize + " OFFSET " + (page - 1) * pageSize;
    }
