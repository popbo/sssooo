                    ...
                    for (DateFormatCondition dateFormatCondition: superRequest.getFieldCondition().getDateFormatConditionS()) {
                        ...
                        if (StringUtils.equals(f.getId(), dateFormatCondition.getFieldId())){
                            ...
                            if (!StringUtil.hasText(dataFormat)){
                                ...
                                switch (dateFormatCondition.getGranularity()){
                                    case DateFormatCondition.YEAR:
                                        dataFormat = MySQLConstants.YEAR_DATE_FORMAT;
                                        break;
                                    case DateFormatCondition.QUARTER:
                                        dataFormat = String.format(MySQLConstants.QUARTER_DATE_FORMAT, originField);
                                        break;
                                    case DateFormatCondition.MONTH:
                                        dataFormat = MySQLConstants.MONTH_DATE_FORMAT;
                                        break;
                                    case DateFormatCondition.WEEK:
                                        dataFormat = MySQLConstants.WEEK_DATE_FORMAT;
                                        break;
                                    case DateFormatCondition.DAY:
                                        dataFormat = MySQLConstants.DAY_DATE_FORMAT;
                                        break;
                                    case DateFormatCondition.HOUR:
                                        dataFormat = MySQLConstants.HOUR_DATE_FORMAT;
                                        break;
                                    case DateFormatCondition.MINUTE:
                                        dataFormat = MySQLConstants.MINUTE_DATE_FORMAT;
                                        break;
                                    case DateFormatCondition.ACCURATE:
                                        dataFormat = MySQLConstants.DEFAULT_DATE_FORMAT;
                                        break;
                                    default:
                                        break;
                                }
        ...
        for (ChartFieldCustomFilterDTO request : requestList) {
            ...
            if (field.getDeType() == 1) {
                ...
                if (field.getDeExtractType() == 1) {
                    if (measureField.contains(field.getId())) {
                        ...
                        whereName = String.format(MySQLConstants.SUM, String.format(MySQLConstants.CAST, originName, MySQLConstants.DEFAULT_INT_FORMAT));
                    } else {
                        //默认 `tableAlias`.`fieldAlias`
            ...
            for (ChartCustomFilterItemDTO filterItemDTO : filter) {
                ...
                if (whereTerm.contains("IN") || whereTerm.contains("in")) {
                    ...
                    whereValue = String.format(MySQLConstants.WHERE_VALUE_BETWEEN, betweenValueS[0], betweenValueS[1]);
                } else {
                    //删除 一些where子句条件
...
//					} else
                    if (StringUtils.containsIgnoreCase(filterItemDTO.getTerm(), "in")) {
                        ...
                        whereValue = String.format(MySQLConstants.WHERE_VALUE_BLANK, value);
                        ;
                    } else {
                        //转换 `whereValue`
