...
public class DMQueryProvider extends QueryProvider {
	...
	public String setSchema(String table, Datasource ds) {
		if (ds != null && !table.startsWith("(") && !table.endsWith(")")) {
			String schema = JsonUtil.toJavaObj(ds.getConfiguration(), JdbcConfiguration.class).getSchema();
			schema = String.format(DMConstants.KEYWORD_TABLE, schema);
			table = schema + "\"" + "." + "\""+ table;
		}
		return table;
	}
...
//						fieldName = String.format(MySQLConstants.STR_TO_DATE, originField, dataFormat);
		...
		return st_sql.render();
		//修改where子句度量条件bug，新增度量聚合条件 2022年3月24日 去除 参数度量条件，参数度量条件不使用sql进行筛选
//		String render = "("+st_sql.render()+")";
//		return creatSqlWithMeasureFilter(render,fields,isGroup,ds,fieldCustomFilter);
	}
	/**
...
//			判断过滤类型 filterType != enum 即代表 其他过滤语句
	...
	private String calcFieldRegex(String originField, SQLObj tableObj) {
		...
		queryWrapper.in("id", new ArrayList<>(ids));
