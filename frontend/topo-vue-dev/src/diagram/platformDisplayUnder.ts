...
export function platformDisplayUnder(ctx: CanvasRenderingContext2D, pen: any) {
  ...
  function drawStright(color) {
    ctx.beginPath();
    ctx.fillStyle = color;
    ctx.fillRect(x + (w * 42) / 249, y, lineL, lineW);
    ctx.closePath();
  }
  function drawDash(color) {
    let rw = lineL;
    const gap = rw / 30;
    ctx.clearRect(x + (w * 42) / 249, y, lineL, lineW);
    ctx.beginPath();
    ctx.fillStyle = color;
    ctx.fillRect(x + (w * 42) / 249, y, lineL, lineW);
    ctx.closePath();
    ...
    for (var i = 0; i < 5; i++) {
      ...
      ctx.fillRect(x + Sx, y, gap * 2, lineW);
    }
    ctx.closePath();
  }
  function drawRoundString(color) {
    ...
    // ctx.clearRect(rx + lineL, ry, r, 2 * r);
    ctx.beginPath();
    ctx.fillStyle = yellowColor;
