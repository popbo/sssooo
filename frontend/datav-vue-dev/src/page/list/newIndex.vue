...
import { importFull } from '@/api/visual'
...
export default {
  ...
  methods: {
    ...
    //导入数据
    importData(content) {
      content.category = this.activeName
      console.log('activeName', content, this.activeName)
      // const importParams={
      //   config:{
      ...
      //   }
      // }
      importFull(content, content.file).then(res => {
        if (res.data.success) {
          this.getList()
        } else {
    ...
    importDataList(list) {
      list.forEach(item => {
        ...
        formDate.set('file', item.raw)
        request({
          url: url + '/visual/full/import/' + this.activeName + '/' + item.name,
          method: 'post',
          headers: {
          ...
          if (res.data.success) {
            ...
            this.$message.warning(`${item.name}导入失败！`)
          }
        })
      })
    },
    ...
    handleBatchLightExport(){
      this.$confirm('是否确认批量导出?', '提示', {
        ...
        this.$nextTick(() => {
          try {
            lightExportObj(checkIdsArr.join(',')).then(res => {
              console.log('批量导出结果', res)
              if (res.data.code==200) {
                window.location.href = res.data.data
                this.getList()
