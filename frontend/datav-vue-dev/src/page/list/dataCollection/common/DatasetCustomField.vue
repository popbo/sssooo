<template>
  <el-col>
    <el-row style="height: 25px;margin-top:7px">
      <span class="table-name title-text" style="color:#bfbfbf">{{ table.name }}</span>
    </el-row>
    <el-divider />
    <el-checkbox v-model="checkAll" :disabled="!(fields.length > 0)" :indeterminate="isIndeterminate" style="font-weight: 400;" @change="handleCheckAllChange">全选</el-checkbox>
...
import { fieldList } from '@/api/dataCollection'
export default {
  ...
  watch: {
    'table': function() {
      fieldList(this.table.id).then(response => {
        this.fields = JSON.parse(JSON.stringify(response.data.data)).filter(ele => ele.extField === 0)
