...
(function (global) {
    ...
    tools.loopShowTooltip = function (chart, chartOption, options) {
      ...
      if(options.timeTicket){
        clearInterval(options.timeTicket);
        options.timeTicket = null;
      }
      if (seriesLen === 0) {
        return;
      }
      ...
      if (options) {
        ...
        options.timeTicket = options.timeTicket || defaultOptions.timeTicket;
      } else {
      ...
      function autoShowTip() {
        ...
        showTip();
        options.timeTicket = setInterval(()=>{
            showTip()
        },options.interval)
      }
